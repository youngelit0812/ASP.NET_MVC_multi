...
namespace MultiFamilyPortal.SaaS.Authentication
{
    ...
    public abstract class OAuthHandler<TOptions> : RemoteAuthenticationHandler<TOptions> where TOptions : OAuthOptions, new()
    {
        protected ITenantSettingsContext _context { get; }
        ...
        /// </summary>
        /// <inheritdoc />
        public OAuthHandler(ITenantSettingsContext context, IOptionsMonitor<TOptions> options, ILoggerFactory logger, UrlEncoder encoder, ISystemClock clock)
        ...
        protected virtual async Task<OAuthTokenResponse> ExchangeCodeAsync(OAuthCodeExchangeContext context)
        {
            ...
            {
                { "client_id", await _context.GetSettingAsync<string>(ClientIdKey) },
                { "redirect_uri", context.RedirectUri },
                { "client_secret", await _context.GetSettingAsync<string>(ClientSecretKey) },
                { "code", context.Code },
        ...
        protected virtual async Task<string> BuildChallengeUrl(AuthenticationProperties properties, string redirectUri)
        {
